...
async function main() {
  const parsed = parseClarifyCliArgs(rawCliArgs);
  ...
  if (!runArg) {
    const errPayload = {
      ...
      error: {
        ...
        message:
          'Uso: npm run clarify -- --run <runId|caminho-output> [--refine] [--approve|--reject] [--approval-notes "texto"] [--skip-llm] [--answers <ficheiro>] [--answer id=valor]... [--overwrite] [--json]',
      },
    };
    if (parsed.json) {
