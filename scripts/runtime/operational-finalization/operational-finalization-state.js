...
const OPERATIONAL_FINALIZATION_STATE_FILE = "operational-finalization-state.json";

const VALID_STATUSES = new Set(["pending", "finalized", "adjustment_requested"]);

/**
 * @param {{
 *   status: "pending"|"finalized"|"adjustment_requested",
 ...
 * }} p
 */
function buildOperationalFinalizationState(p) {
 ...
 */
function validateOperationalFinalizationState(obj) {
  /** @type {string[]} */
  const errors = [];
 ...
 */
function loadOperationalFinalizationState(outputDir) {
  const fp = path.join(
    path.resolve(outputDir),
    OPERATIONAL_FINALIZATION_STATE_FILE,
  );
  if (!fs.existsSync(fp)) return { ok: false };
  try {
    const doc = JSON.parse(fs.readFileSync(fp, "utf-8"));
    const v = validateOperationalFinalizationState(doc);
 ...
 * @param {object} doc
 */
function writeOperationalFinalizationState(outputDir, doc) {
  const fp = path.join(
    path.resolve(outputDir),
    OPERATIONAL_FINALIZATION_STATE_FILE,
  );
 ...
 */
function ensureOperationalFinalizationState(outputDir) {
  const loaded = loadOperationalFinalizationState(outputDir);
  if (loaded.ok) return loaded;
  const doc = buildOperationalFinalizationState({ status: "pending" });
